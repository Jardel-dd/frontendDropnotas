...
import { SetErrorsFn } from '../types/perfilUsuario';
...
export const fetchFilteredPerfilUsuarios = async (filtro: string): Promise<PerfilUser[]> => {
    try {
        const response = await api.get('/perfil-usuario', {
         ...
         console.log("Perfil de ususario:", response.data);
